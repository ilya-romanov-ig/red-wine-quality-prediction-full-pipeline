...
class ModelTrainer:
    def __init__(self, is_logged=False, cv_folds=5, logger_config=None):
        self.model = None
        self.cv_folds = cv_folds
        self.cv_res = None
        ...
        if is_logged:
            self.mlflow_mgr = MLflowManager(
                experiment_name=logger_config.get('experiment_name', 'Default name'),
                tracking_uri=logger_config.get('tracking_uri', None)
            )
        
    def train(self, X_train, X_test, y_train, y_test, use_cv, use_gridsearch, params)
